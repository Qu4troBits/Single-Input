...
final class SaveProjectionRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()->can('save-financial-projection');
    }
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'items.*.type' => ['required', 'in:revenue,expense,profit,cash_flow,balance_sheet'],
            'items.*.category_id' => ['nullable', 'string', 'max:36'],
            'items.*.category_name' => ['nullable', 'string', 'max:100'],
            'items.*.notes' => ['nullable', 'string', 'max:500'],
            'items.*.source' => ['nullable', 'in:historical,manual,formula'],
        ];
    ...
    public function messages(): array
    {
        return [
            ...
            'items.*.type.in' => 'Tipo de item inválido.',
